...
; The implications case: +avx512f (not +fma explicitly) should still report
; fma=true because the subtarget infers it.
...
; A function with -fma should NOT have FMA available.
; CHECK-LABEL: @no_fma
; CHECK-NOT: julia.cpu.supports
...
; Hyphenated feature names (sse4.2) work.
; CHECK-LABEL: @sse42
; CHECK-NOT: julia.cpu.supports
; CHECK: call i32 @sink_true
define i32 @sse42() #3 {
...
; Unknown feature name folds to false (and does not vacuously match via
; LLVM's checkFeatures), even when the function's target-features look
; "rich". This guards direct Core.Intrinsics.cpu_supports calls; the
; Base.@cpu_supports macro normally validates the name at parse time.
