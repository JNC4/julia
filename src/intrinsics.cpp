...
static jl_cgval_t emit_intrinsic(jl_codectx_t &ctx, intrinsic f, jl_value_t **args, size_t nargs)
{
    ...
    case cpu_supports: {
        ...
        // Reject empty / non-printable / containing '.' or ',' to keep the
        // intrinsic name parseable. The Julia wrapper validates feature names
        // against the cpufeatures table; this is defense-in-depth.
        if (*feat == '\0')
            return emit_runtime_call(ctx, f, argv, nargs);
